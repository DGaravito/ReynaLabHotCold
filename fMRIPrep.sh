...
do
	...
	then
      ...
      3dTcat -prefix sub-${sub}_task-framing_${run}_bold_No2.nii sub-${sub}_task-framing_${run}_bold.nii'[2..$]'
			echo "Removing original to maintain BIDS structure"
			rm sub-${sub}_task-framing_run-${run}_bold.nii
			echo "Renaming new to match original"
			mv sub-${sub}_task-framing_${run}_bold_No2.nii sub-${sub}_task-framing_${run}_bold.nii
		done
